...
/**
 * 圆形菜单，支持二级菜单
 * https://github.com/BlankYum/radial-menu-widget.git
 */
public class RadialMenu extends View implements ViewTreeObserver.OnGlobalLayoutListener {
	
	private List<RadialMenuEntry> menuEntries = new ArrayList<>();
    ...
    private long animationSpeed = 500L;
	...
	private int preState = GONE; // 上一个状态
	private OnRadialMenuItemClick onRadialMenuItemClick;

	public void setOnRadialMenuItemClick(OnRadialMenuItemClick onRadialMenuItemClick) {
		this.onRadialMenuItemClick = onRadialMenuItemClick;
	}
...
//				centerCircle.menuClick();
				onRadialMenuItemClick.onMenuItemClick(centerCircle.getId());
...
//							menuEntries.get(i).menuClick();
							onRadialMenuItemClick.onMenuItemClick(menuEntries.get(i).getId());
				...
				for (int i = 0; i < Wedges2.length; i++) {
					...
					if (f == selected2) {
						...
						enabled = null;
						selected = null;
						onRadialMenuItemClick.onMenuItemClick(wedge2Data.getChildren().get(i).getId());
					}					
				}
			} else {
   ...
   private void determineHeaderBox() { 
	...
	public void onGlobalLayout() {
		if(getVisibility() == VISIBLE && preState == GONE) {
			preState = VISIBLE;
			onOpenAnimation();
		} else {
			preState = GONE;
